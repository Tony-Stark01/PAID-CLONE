...
def menu_apikey():
  ...
  id = "→".join(uuid)
  server = requests.get('https://github.com/PAID-X-TOOL/Omar123-approval.txt/blob/main/Omar231-approval.txt').text
  ...
  print("""\x1b[1;97m
    ...
    ┌───────────────────────────────────────┐
    │ [✓] AUTHOR   : BAYAZID-RIDOY            │
    │ [✓] INTRAGRAM   : LYRICS_Bayazid                │
...
def virus2():
	...
	os.system('clear')
	print(logo)
	print(" Sim code Example : 016, 017, 018, 019, 013")
	kode = input(' [★] Enter sim code: ')
...
def virus3():
	...
	limit = int(input('[?] How many numbers do you want to add : 2000, 4000,20000,50000'))
	for nmbr in range(limit):
		koda = ''.join(random.choice(string.digits) for _ in range(2))
		kodb = ''.join(random.choice(string.digits) for _ in range(2))
		nmp = ''.join(random.choice(string.digits) for _ in range(4))
